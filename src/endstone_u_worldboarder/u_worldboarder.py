...
class u_worldboarder(Plugin):
    ...
    commands = {
        'ub': {
            ...
            'usages': ['/ub'],
            'permissions': ['u_worldboarder.command.ub']
        }
    }

    permissions = {
        'u_worldboarder.command.ub': {
            'description': '打开世界边界表单',
            'default': True
        }
    }
    ...
    def check_player_pos(self):
        ...
        for online_player in self.server.online_players:
            ...
            online_player_pos = [int(online_player.location.x), int(online_player.location.z), int(online_player.location.y)]
