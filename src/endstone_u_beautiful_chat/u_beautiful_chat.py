...
class u_beautiful_chat(Plugin):
    ...
    def on_enable(self):
        ...
        if not os.path.exists(config_data_file_path):
            config_data = {
                ...
                'player_join_message': '§l§a[+] §r{0} 加入了游戏...',
                'player_left_notice_sound': 'note.guitar',
                'player_left_message': '§l§c[-] §r{0} 暂时离开了...',
            ...
            with open(config_data_file_path, 'r', encoding='utf-8') as f:
                ...
                if not pre_config_data.get('player_join_message'):
                    pre_config_data['player_join_message'] = '§l§a[+] §r{0} 加入了游戏...'
                if not pre_config_data.get('player_left_notice_sound'):
                    pre_config_data['player_left_notice_sound'] = 'note.guitar'
                if not pre_config_data.get('player_left_message'):
                    pre_config_data['player_left_message'] = '§l§c[-] §r{0} 暂时离开了...'
                if not pre_config_data.get('allow_nick_name'):
                    pre_config_data['allow_nick_name'] = True
        ...
        if not os.path.exists(mute_data_file_path):
            mute_data = []
            with open(mute_data_file_path, 'w', encoding='utf-8') as f:
                json_str = json.dumps(mute_data_file_path, indent=4, ensure_ascii=False)
                f.write(json_str)
        else:
            with open(bad_words_data_file_path, 'r', encoding='utf-8') as f:
    ...
    def on_command(self, sender: CommandSender, command: Command, args: list[str]):
        if command.name == 'ubc':
            ...
            ubc_main_form.add_button(f'{ColorFormat.YELLOW}设置称号', icon='textures/ui/emptyStar', on_click=self.set_nick_name)
            if player.is_op == True:
                ...
                ubc_main_form.add_button(f'{ColorFormat.YELLOW}禁言/解除禁言玩家', icon='textures/ui/comment', on_click=self.mute)
            ...
            if not os.path.exists(menu_data_file_path):
                ...
                ubc_main_form.on_close = None
            else:
                ubc_main_form.add_button(f'{ColorFormat.YELLOW}返回', icon='textures/ui/refresh_light', on_click=self.back_to_menu)
                ubc_main_form.on_close = self.back_to_menu
            player.send_form(ubc_main_form)

    # 设置个人称号
    def set_nick_name(self, player: Player):
        if self.config_data['allow_nick_name'] == False:
            player.send_message(f'{ColorFormat.RED}称号设置失败: {ColorFormat.WHITE}当前服务器静止玩家设置称号...')
            return
    ...
    # 禁言/解除禁言玩家
    def mute(self, player: Player):
        mute_form = ActionForm(
            title=f'{ColorFormat.BOLD}{ColorFormat.LIGHT_PURPLE}禁言/解除禁言玩家',
            content=f'{ColorFormat.BOLD}请选择操作...',
            on_close=self.back_to_main_form
        )
        mute_form.add_button(f'{ColorFormat.YELLOW}禁言玩家', icon='textures/ui/mute_on', on_click=self.on_mute)
        mute_form.add_button(f'{ColorFormat.YELLOW}解除禁言玩家', icon='textures/ui/mute_off', on_click=self.unmute)
        mute_form.add_button(f'{ColorFormat.YELLOW}返回', icon='textures/ui/refresh_light', on_click=self.back_to_main_form)
        player.send_form(mute_form)

    # 禁言玩家
    def on_mute(self, player: Player):
        player_name_list = [key for key in self.nick_name_data.keys() if key not in self.mute_data and key != player.name]
        if len(player_name_list) == 0:
            player.send_message(f'{ColorFormat.RED}禁言失败： {ColorFormat.WHITE}经排查, 没有可禁言的玩家...')
            return
        dropdown = Dropdown(
            label=f'{ColorFormat.GREEN}请选择玩家（已排除已禁言玩家）...',
            options=player_name_list
        )
        on_mute_form = ModalForm(
            title=f'{ColorFormat.BOLD}{ColorFormat.LIGHT_PURPLE}禁言玩家',
            controls=[dropdown],
            on_close=self.mute,
            submit_button=f'{ColorFormat.YELLOW}禁言'
        )
        def on_submit(player: Player, json_str):
            data = json.loads(json_str)
            target_mute_player_name = player_name_list[data[0]]
            self.mute_data.append(target_mute_player_name)
            self.save_mute_data()
            player.send_message(f'{ColorFormat.YELLOW}禁言玩家成功...')
        on_mute_form.on_submit = on_submit
        player.send_form(on_mute_form)

    # 解除禁言玩家
    def unmute(self, player: Player):
        already_mute_player_list = [mute_player_name for mute_player_name in self.mute_data]
        if len(already_mute_player_list) == 0:
            player.send_message(f'{ColorFormat.RED}解除禁言失败： {ColorFormat.WHITE}当前没有任何被禁言的玩家...')
            return
        dropdown = Dropdown(
            label=f'{ColorFormat.GREEN}请选择玩家...',
            options=already_mute_player_list
        )
        unmute_form = ModalForm(
            title=f'{ColorFormat.BOLD}{ColorFormat.LIGHT_PURPLE}解除禁言玩家',
            controls=[dropdown],
            on_close=self.mute,
            submit_button=f'{ColorFormat.YELLOW}解除禁言'
        )
        def on_submit(player: Player, json_str):
            data = json.loads(json_str)
            target_unmute_player_name = already_mute_player_list[data[0]]
            self.mute_data.remove(target_unmute_player_name)
            self.save_mute_data()
            player.send_message(f'{ColorFormat.YELLOW}解除禁言玩家成功...')
        unmute_form.on_submit = on_submit
        player.send_form(unmute_form)
    ...
    def set_config_data(self, player: Player):
        ...
        textinput1 = TextInput(
            label=f'{ColorFormat.GREEN}当前变量显示设置：\n'
                  ...
                  f'{ColorFormat.GREEN}输入新的变量显示设置（留空则保留原配置）',
            placeholder='例如： money 或 money++health++ping 或 clear'
        )
        toggle = Toggle(
            label=f'{ColorFormat.GREEN}允许玩家设置称号'
        )
        if self.config_data['allow_nick_name'] == True:
        ...
        textinput2 = TextInput(
            label=f'\n'
                  ...
                  f'{ColorFormat.GREEN}输入新的允许称号最大字数（留空则保留原配置）',
        ...
        textinput3 = TextInput(
            label=f'\n'
                  ...
                  f'{ColorFormat.GREEN}输入新的玩家更新称号耗费（留空则保留原配置）',
        ...
        textinput4 = TextInput(
            label=f'\n'
                  ...
                  f'{ColorFormat.GREEN}输入新的玩家进服音效（留空则保留原配置）',
            placeholder='请输入音效代码, 例如：note.bell'
        )
        textinput5 = TextInput(
            label=f'\n'
                  f'{ColorFormat.GREEN}当前玩家进服提示信息： {ColorFormat.WHITE}{self.config_data['player_join_message']}\n'
                  f'{ColorFormat.GREEN}输入新的玩家进服提示信息（留空则保留原配置）\n',
            placeholder='请输入任意字符串, 务必保留占位符 {0}'
        )
        textinput6 = TextInput(
            label=f'\n'
                  f'{ColorFormat.GREEN}当前玩家退服音效： {ColorFormat.WHITE}{self.config_data['player_left_notice_sound']}\n'
                  f'{ColorFormat.GREEN}输入新的玩家退服音效（留空则保留原配置）\n',
            placeholder='请输入音效代码, 例如：note.bell'
        )
        textinput7 = TextInput(
            label=f'\n'
                  f'{ColorFormat.GREEN}当前玩家退服提示信息： {ColorFormat.WHITE}{self.config_data['player_left_message']}\n'
                  f'{ColorFormat.GREEN}输入新的玩家退服提示信息（留空则保留原配置）\n',
            placeholder='请输入任意字符串, 务必保留占位符 {0}'
        )
        set_config_data_form = ModalForm(
            title=f'{ColorFormat.BOLD}{ColorFormat.LIGHT_PURPLE}重载配置文件',
            controls=[textinput1, toggle, textinput2, textinput3, textinput4, textinput5, textinput6, textinput7],
        ...
        def on_submit(player: Player, json_str):
            ...
            try:
                ...
                if data[1] == True:
                    self.config_data['allow_nick_name'] = True
                else:
                    self.config_data['allow_nick_name'] = False
                if len(data[2]) == 0:
                    new_nick_name_len = self.config_data['nick_name_len']
                else:
                    new_nick_name_len = int(data[2])
                if len(data[3]) == 0:
                    new_update_nick_name_cost = self.config_data['update_nick_name_cost']
                else:
                    new_update_nick_name_cost = int(data[3])
                if len(data[4]) == 0:
                    new_player_join_notice_sound = self.config_data['player_join_notice_sound']
                else:
                    new_player_join_notice_sound = data[4]
                if len(data[5]) == 0:
                    new_player_join_message = self.config_data['player_join_message']
                else:
                    new_player_join_message = data[5]
                if len(data[6]) == 0:
                    new_player_left_notice_sound = self.config_data['player_left_notice_sound']
                else:
                    new_player_left_notice_sound = data[6]
                if len(data[7]) == 0:
                    new_player_left_message = self.config_data['player_left_message']
                else:
                    new_player_left_message = data[7]
            except:
                player.send_message(f'{ColorFormat.RED}表单解析错误, 请按提示正确填写...')
                return
            ...
            self.config_data['player_join_message'] = new_player_join_message
            self.config_data['player_left_notice_sound'] = new_player_left_notice_sound
            self.config_data['player_left_message'] = new_player_left_message
            self.save_config_data()
    ...
    # 获取玩家生存模式
    def get_player_mode(self, player: Player):
        pre_player_mode = player.game_mode.name
        if pre_player_mode == 'CREATIVE':
            player_mode = f'{ColorFormat.YELLOW}创造'
        elif pre_player_mode == 'SURVIVAL':
            player_mode = f'{ColorFormat.GREEN}生存'
        else:
            player_mode = f'{ColorFormat.RED}冒险'
        return player_mode
    ...
    def on_player_chat(self, event: PlayerChatEvent):
        if event.player.name in self.mute_data:
            event.player.send_message(f'{ColorFormat.RED}你已被禁言...')
        ...
        if len(self.config_data['variable_order']) == 0:
            ...
            for variable in variable_order_list:
                if variable == 'dim':
                    ...
                    variable_to_add = self.get_player_mode(event.player)
                else:
    ...
    def on_player_join(self, event: PlayerJoinEvent):
        ...
        self.save_nick_name_data()
        event.join_message = self.config_data['player_join_message'].format(event.player.name)
    ...
    def on_player_left(self, event: PlayerQuitEvent):
        event.quit_message = self.config_data['player_left_message'].format(event.player.name)
        self.server.dispatch_command(self.CommandSenderWrapper,f'playsound {self.config_data['player_left_notice_sound']} @a')