...
class utransfer(Plugin):
    ...
    def on_enable(self):
        ...
        try:
            f = open(server_file_path, 'r', encoding='utf-8')
            server_list = json.loads(f.read())
            f.close()
        except:
            f = open(server_file_path, 'w', encoding='utf-8')
            default_server_list = [
                {
                'name': 'Misaki',
                'ip': 'play.misaki.host',
        ...
        try:
            f1 = open(config_file_path, 'r', encoding='utf-8')
            config_data= json.loads(f1.read())
            f1.close()
        except:
            f1 = open(config_file_path, 'w', encoding='utf-8')
            default_config_data = {
        ...
        self.task = self.server.scheduler.run_task(self, self.update_server_status, delay=0, period=20)
    ...
    def on_command(self, sender: CommandSender, command: Command, args: list[str]):
        ...
        if command.name == 'tr':
            server_list_form = ActionForm(
                title=self.config_data['server_form_title'],
                content=self.config_data['server_form_content']
            )
    ...
    def on_confirm(self, server_name, server_ip, server_port):
        def on_click(player: Player):
            confirm_form = ActionForm(
                title=self.config_data['confirm_form_title'],
                content=self.config_data['confirm_form_content']
            )
    ...
    def reload_config_data(self, player: Player):
        if player.is_op == False:
            player.send_message(f'{ColorFormat.RED}你没有权限执行此操作...')
            return
        else:
            f = open(server_file_path, 'r', encoding='utf-8')
            ...
            f.close()
            f1 = open(config_file_path, 'r', encoding='utf-8')
            self.config_data = json.loads(f1.read())
            f1.close()
