...
from endstone.event import event_handler, PlayerJoinEvent, BlockBreakEvent, ActorKnockbackEvent, PlayerInteractEvent, PlayerInteractActorEvent, ActorSpawnEvent
...
class uland(Plugin):
    ...
    def on_command(self, sender: CommandSender, command: Command, args: list[str]):
        ...
        if command.name == 'posa':
            ...
            if not self.record_create_land_event.get(player.name):
                ...
                return
            # 记录玩家选中的A点坐标 x 和 z
            PosA = [int(player.location.x), int(player.location.z), int(player.location.y)]
        ...
        if command.name == 'posb':
            ...
            if self.record_create_land_event[player.name]['dimension'] != player.location.dimension.name:
                ...
                return
            # 记录玩家选中的B点坐标 x 和 z
            PosB = [int(player.location.x), int(player.location.z)]
            ...
            if self.record_create_land_event[player.name].get('PosB'):
                self.record_create_land_event[player.name]['PosB'] = PosB
                player.send_message(f'{ColorFormat.YELLOW}B点更新成功, 坐标 ({PosB[0]}, ~, {PosB[1]})')
            else:
                self.record_create_land_event[player.name]['PosB'] = PosB
                player.send_message(f'{ColorFormat.YELLOW}B点选中成功, 坐标 ({PosB[0]}, ~, {PosB[1]})')
            # 测试用代码...
            self.logger.info(f'{PosB}')
    ...
    def my_land_set_land_tp(self, land_name):
        def on_click(player: Player):
            new_land_tp = [int(player.location.x), int(player.location.y), int(player.location.z)]
    ...
    def my_land_sell(self, land_name):
        def on_click(player: Player):
            ...
            if self.config_data['is_land_sell_rate_on'] == False:
                ...
                land_sell_money = int(land_expense * land_sell_rate)
    ...
    def on_block_break(self, event: BlockBreakEvent):
        block_pos = [int(event.block.location.x), int(event.block.location.z)]
    ...
    def on_mob_spawn(self, event: ActorSpawnEvent):
        ...
        if event.actor.name == 'Ender Crystal' or event.actor.name == 'Lightning Bolt':
            actor_pos =[int(event.actor.location.x), int(event.actor.location.z)]
            ...
            for value in self.land_data.values():
                ...
                for value in land.values():
                    ...
                    for i in it:
                        range.append(int(i.group()))
                    land_len_x = int(abs(range[0] - range[2]) / 2)
                    land_len_z = int(abs(range[1] - range[3]) / 2)
    ...
    def on_player_attack(self, event: ActorKnockbackEvent):
        mob_under_attack_pos = [int(event.actor.location.x), int(event.actor.location.z)]
    ...
    # 领地 TNT 防爆, 防 Creeper, wither周期任务函数
