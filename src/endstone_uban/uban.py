...
class uban(Plugin):
    ...
    def on_enable(self):
        ...
        # 加载屏蔽词列表数据
        if not os.path.exists(badwords_list_file_path):
            badwords_list_data = []
            with open(badwords_list_file_path, 'w', encoding='utf-8') as f:
                json_str = json.dumps(badwords_list_data, indent=4, ensure_ascii=False)
                f.write(json_str)
        else:
            with open(badwords_list_file_path, 'r', encoding='utf-8') as f:
                badwords_list_data = json.loads(f.read())
    ...
    def on_command(self, sender: CommandSender, command: Command, args: list[str]):
        if command.name == 'uban':
            ...
            if player.is_op == True:
                ...
                main_form_op.add_button(f'{ColorFormat.YELLOW}查看封禁列表', icon='textures/ui/lock_color', on_click=self.list_banlist)
                main_form_op.add_button(f'{ColorFormat.YELLOW}屏蔽不文明用语', icon='textures/ui/comment', on_click=self.ban_badwords)
                ...
                mian_form_player.add_button(f'{ColorFormat.YELLOW}查看封禁列表', icon='textures/ui/lock_color', on_click=self.list_banlist)
    ...
    def ban_badwords(self, p: Player):
        ...
        ban_badwords_form = ModalForm(
            title=f'{ColorFormat.BOLD}{ColorFormat.LIGHT_PURPLE}添加不文明用语表单',
            controls=[textinput],
            submit_button=f'{ColorFormat.GREEN}确认',
            on_close=self.back_to_main_form
        )
        def on_submit(p: Player, json_str):
            data = json.loads(json_str)
            ...
            p.send_message(f'{ColorFormat.RED}屏蔽词 {ColorFormat.WHITE}{data[0]} {ColorFormat.RED}已添加...')
